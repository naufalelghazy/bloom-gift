  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center p-4 z-20 overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full max-w-[340px] bg-[#faf8f5] border border-[#eedcc5] rounded-[32px] shadow-xl p-6 relative flex flex-col items-center pt-24 pb-8 transition-all duration-300 min-h-[480px] justify-between"
      >
        ...
        <motion.div
          ...
          className="flex flex-col items-center w-full h-full flex-1 justify-between gap-4"
        >
          {/* Title / Question / Hint message */}
          <div className="text-center space-y-1.5 select-none z-10 w-full px-2">
            <h3 className="font-garamond italic text-[20px] leading-snug text-[#7c5e49] tracking-wide font-medium">
              {isQuestion ? 'Pertanyaan untukmu 💌' : (security.hint || 'hayolo, apa yaa kode nyaa')}
            </h3>
            {isQuestion && security.question && (
              <div className="space-y-2 mt-2">
                <p className="font-sans text-sm text-[#9c6644] font-medium leading-relaxed bg-[#eedcc5]/20 border border-[#eedcc5]/40 rounded-xl px-4 py-3">
                  {security.question}
                </p>
                {security.hint && (
                  <p className="text-[11px] text-[#a48c82] italic font-sans">
                    Petunjuk: {security.hint}
                  </p>
                )}
              </div>
            )}
          ...
          {isQuestion ? (
            <form onSubmit={handleFormSubmit} className="w-full flex-1 flex flex-col items-center justify-between z-10 mt-1 py-2">
              <div className="w-full flex flex-col items-center gap-3">
                ...
                />
                {isError && (
                  <p className="text-red-500 text-[11px] font-sans">
              ...
              <button
                type="submit"
                className="w-full max-w-[270px] py-3 rounded-2xl bg-[#9c6644] hover:bg-[#855333] text-[#faf8f5] font-medium text-sm transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md shadow-amber-900/10 mt-auto"
              >
                Buka Kado
              </button>
            </form>
